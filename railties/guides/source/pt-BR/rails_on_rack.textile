...
O Rails fornece uma interface simples de configuração +config.middleware+ para adicionar, remover e modificar os middlewares na pilha de middlewares por meio do +environment.rb+ ou do arquivo de configuração específico do ambiente <tt>environments/&lt;ambiente&gt;.rb</tt>.
...
Cada string no array deve ser o nome da sua classe metal. Se você fizer isso, será avisado de que aplicações metal não listadas não serão carregadas.
