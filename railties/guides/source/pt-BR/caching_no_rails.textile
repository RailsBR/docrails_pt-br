h2. Caching no Rails: Visão Geral

Este guia irá ensinar-lhe o que precisa saber para evitar consultas 
custosas ao seu banco de dados mas ainda retornando o que você precisa 
retornar a esses famintos clientes web na menor fração de tempo possível.

endprologue.

h3. Caching Básico

Este é uma introdução aos três tipos de técnicas para caching que o Rails
provê por padrão sem nenhum plugin de terceiros.

Para iniciar tenha certeza que config.action_controller.perform_caching está
...
por padrão no ambiente development e test, e habilitado em production.
...
Page caching é um mecanismo do Rails que permite que a requisição por páginas
...
Então, como ativar este comportamente super rápido de cache? Simples, imagine que você
