...
* Mais Informações (neste ponto o método de conveniência era chamado de +each+):
...
* Counts em scoped selects agora funcionam corretamente, assim você pode fazer coisas como +Account.scoped(:select => "DISTINCT credit_limit").count+.
...
Além disso, você pode utilizar o mesmo schema para localizar os arquivos erro no diretório +public+: +public/500.da.html+ ou +public/404.en.html+ funcionam, por exemplo.
...
É provável que você conheca a prática do Rails de adicionar timestamps para caminhos estáticos de assets como um "cache buster". Isto ajuda a garantir que cópias antigas de coisas como imagens e stylesheets não sejam servidos para a cache do navegador dos usuários quando você os modifica no servidor. Agora você pode modificar este comportamento com a opção de configuração +cache_asset_timestamps+ para o Action View. Se você ativar a cache, o Rails calculará o timestamp uma vez quando ele servir um asset a primeira vez, e salvará este valor. Isto significa algumas (custosas) chamadas ao sistema de arquivos para servir assets estáticos - mas também significa que você não pode modificar nenhum dos assets enquanto o servidor está executando e esperar que as mudanças sejam obtidas pelos clientes.
...
h4. Tags com Opção Disabled para Form Select Helpers

Os form select helpers (tais como +select+ e +options_for_select+) agora suportam uma opção +:disabled+, que pode levar um único valor ou um array de valores para disabilitar as tags resultantes:
...
O suporte para parsing XML no ActiveSupport tornou-se mais flexível permitindo que você troque entre diferentes parsers. Por padrão, ele utiliza a implementação REXML padrão, mas você pode facilmente especificar as implementações mais rápidas LibXML ou Nokogiri para suas próprias aplicações, contanto que você tem as gems apropriadas instaladas:
...
* +ActiveSupport::MessageEncryptor+ fornece uma maneira simples para encriptar informação para armazenamento em uma localização não confiável (como cookies).
...
Rails Metal é um novo mecanismo que proporciona endpoints super rápidos dentro de suas aplicações Rails. As classes Metal passam por cima do roteamento e do Action Controller para lhe dar velocidade pura (ao custo de todas as coisas no Action Controller, claro). Isto baseia-se em todo o trabalho base recente para tornar o Rails uma aplicação Rack com uma pilha de middleware exposta. Endpoints Metal podem ser carregados a partir de sua aplicação ou de plugins.
