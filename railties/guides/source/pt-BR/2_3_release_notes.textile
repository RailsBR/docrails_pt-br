...
Existe um par de mundaças significativas no roteamento do Rails 2.3. Os helpers para rotas +formatted_+ sumiram, em favor de passar apenas +:format+ como uma opção. Isto reduz o processo de geração de rotas em 50% para qualquer recurso - e pode salvar uma quantia substancial de memória (até 100MB em grandes aplicações). Se o seu código utiliza os helpers +formatted_+, ele ainda funcionará atualmente - mas este comportamento está depreciado e sua aplicação será mais eficiente se você reescrever estas rotas usando o novo padrão. Outra granda mudança é que o Rails agora suporta vários arquivos de roteamento, não só o +routes.rb+. Você pode utilizar +RouteSet#add_configuration_file+ para importar mais rotas a qualquer momento - sem limpar as rotas carregadas atualmente. Embora esta mudança é mais útil para os Engines, você pode utilizá-la em qualquer aplicação que precise carregar rotas em lote.
...
Alguns pedaços de código antigo foram depreciados nesta versão:
...
* +render_component+ vai de "depreciado" para "naoexistente" no Rails 2.3. Se você ainda precisa dele, pode instalar o "plugin render_component":http://github.com/rails/render_component/tree/master.
...
* +ActionController::Base#session_enabled?+ está depreciado porque as sessões são agora carregadas sob demanda (lazy-load).
* As opções +:digest+ e +:secret+ para o +protect_from_forgery+ estão depreciadas e não tem efeito.
...
* +formatted_polymorphic_url+ está depreciado. Utilize no lugar +polymorphic_url+ com +:format+.
