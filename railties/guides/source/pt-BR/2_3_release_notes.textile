...
h4. Escopos Padrão
 ...
 Rails 2.3 introduzirá a noção de _escopos padrão_ similares aos named scopes, mas aplicados a todos os named scopes ou métodos find dentro do modelo. Por exemplo, você pode escrever +default_scope :order => 'name ASC'+ e em qualquer hora que você recuperar registros daquele modelo eles retornarão ordenados pelo nome (a menos que você sobrescreva a opção, é claro).
