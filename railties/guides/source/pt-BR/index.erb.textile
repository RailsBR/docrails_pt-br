...
<dl>
  ...
  <% guide_pt_br("API de Internacionalização do Rails", 'i18n.html', :revision_ticket => 32) do %>
  Este guia cobre como adicionar internacionalização em sua aplicação. Sua aplicação irá ser capaz de traduzir conteúdo para diferentes idiomas, alterar regras de pluralização, usar formatação correta de datas para cada país e muito mais.
	...
	<% guide_pt_br("Configuring Rails Applications", "configuring.html", :translation_ticket => 31 ) do %>
	  Este guia cobre as opções básicas de configuração para uma aplicação Rails. (Em tradução)
		<p>
		  Você pode ver o original em inglês <%= link_to "aqui", "http://guides.rubyonrails.org/configuring.html" %>
		</p>	
	<% end %>
  ...
  <% guide_pt_br("Contribuindo para o Rails", 'contributing_to_rails.html', :revision_ticket => 34) do %>
