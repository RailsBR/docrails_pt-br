...
Esse guia aborda as várias maneiras de se realizar testes de performance em aplicações Rails. Com dele você será capaz de:
...
Esse é um exemplo de um teste de performance simples, para fazer o profiling de uma requição GET na homepage da aplicação.
...
Você pode encontrar mais detalhes sobre os métodos +get+ e +post+ no link:../testing_rails_applications.html#mgunderloy[Testing Rails Applications] guide.

h5. Exemplo para models

Ainda que os testes de performance seja testes de integração e portanto naturalmente mais próximos do ciclo request/response, você também pode realizar testes de performance somente com código de modelos.
...
##Verificar se isso são ciclos do processador ou coisa do tipo, a explicação não está boa
...
Modo: Benchmarking, no modo Profiling exige a utilização do "Ruby com GC modificado":#installing-gc-patched-ruby
...
Modo: No modo Benchmarking exige a utilização do "Ruby com GC modificado":#installing-gc-patched-ruby
...
Os testes de performance colocam os diversos resultados obtidos no dentro do diretório +tmp/performance+ variando de acordo com o modo e a métrica utilizados.
...
É a principal forma de apresentação dos resultados no modo benchmarking mode. Por exemplo:
...
Essas instruções irão fazer o benchmark do código dentro do bloco +Project.benchmark("Creating project") do...end+ e irá gravar o resultado para no arquivo de log:
...
De maneira semelhante, você pode utilizar esse helper dentro de "controllers":http://api.rubyonrails.com/classes/ActionController/Benchmarking/ClassMethods.html#M000715
