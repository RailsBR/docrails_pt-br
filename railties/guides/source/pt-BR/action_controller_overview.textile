h2. Visão Geral do Action Controller 
...
* Seguir o fluxo de uma requisição em um controller
...
* Filtrar parâmetros críticos para que não apareçam no log da aplicação
...
NOTE: Alterando a secret quando estiver usando CookieStore irá invalidar todas as sessions existentes.

h4. Acessando a Sessão

Em seu controller, você pode acessar a sessão através do método de instância +session+.

NOTE: Sessions são carregadas por demanda (lazy load). Se você não acessar a session no código de sua action, ela não será carregada. Por isso você nunca precisa desativar a session, apenas não acessando-a já fará o serviço.
...
h4. Filtros Posteriores (After Filters) e Filtros Antes e Depois (Around Filters)
...
|_.Propriedade do +request+|_.Propósito|
...
|_.Propriedade do +response+|_.Propósito|
...
WARNING: Tome cuidado quando utilizar (ou simplesmente não utilize) dados "de fora" (parâmetros, cookies, etc) para localizar um arquivo em disco, já que isto é um risco de segurança que pode permitir acesso a arquivos que não deveria.
...
* Março 2, 2009: Revisão da tradução por "Daniel Lopes":translators.html#danielvlopes
