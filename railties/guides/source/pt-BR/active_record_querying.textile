...
Se você estiver acostumado a utilizar SQL puro para encontrar registros no banco de dados então, de forma geral, você descobrirá que existem maneiras melhores de realizar as mesmas operações no Rails. O Active Record evita a necessidade de utilizar SQL na maioria dos casos.

Os exemplos de código por todos este guia irão se referir a um ou mais dos seguintes models:

TIP: Todos os models a seguir utilizar +id+ como chave primária, a não ser quando específicado diferentemente.
...
Utilizando <tt>Model.find(primary_key, options = nil)</tt>, você pode recuperar o objeto que corresponde à chave primária fornecida. Por exemplo:
...
<tt>Model.find(primary_key)</tt> irá lançar uma exceção +ActiveRecord::RecordNotFound+ caso nenhum registro seja encontado.
...
<tt>Model.last(options = nil)</tt> encontra o último registro que case com as opções fornecidas. Se +options+ não for informado, o último registro encontrado seŕa retornado. Por exemplo:
...
Isso acontece porque +User.all+ recupera _toda a tabela_, constrói um objeto por linha, e mantém o array inteiro na memória. Algumas vezes isso pode significar objetos demais e exige memória demais.

h5. +find_each+

Para iterar efientemente sobre uma grande tabela, o Active Record fornece um método para pesquisa em lote chamado  +find_each+:
...
Por baixo dos panos +find_each+ recupera linhas em lotes de +1000+ e retorna-os um por um.
...
Se você quiser adicionar condições ao seu +find+, você pode simplesmente especificá-las ali, como em +Client.first(:conditions => "orders_count = '2'")+. Isso irá encontrar todos os clientes onde o valod do campo +orders_count+ for igual a 2.
...
Agora, o que fazer caso aquele número possa variar, como um argumento vindo de algum outro lugar? O +find+ então se transforma em algo como:
...
Neste exemplo, o primeiro ponto de interrogação será substituído com o valor em +params[:orders]+ e o segundo será substituído com a representação SQL de +false+, a qual depende do adapter sendo utilizado.
...
deve-se à segurança de argumentos. Colocar a variável diretamente dentro da string de condições irá passar o conteúdo da variável para o banco de dados *sem qualquer alteração*. Isso significa que a variável será diretamente informada por um usuário (possivelmente) mal intencionado e não será tratada. Se você fizer isso, você estará colocando toda a sua base de dados em risco porque uma vez que um usuário descubra que ele pode atacar seu banco de dados ele pode fazer praticamente qualquer coisa com isso. Nunca, jamais, coloque seus argumentos diretamente dentro de uma string de condições.
...
h5. Condições de Tempo de Data

As coisas podem ficar *realmente* ruins se você fornecer objetos Time pois isso irá comparar o seu campo com *cada segundo* naquele intervalo:
...
Isso poderia fazer com que seu servidor de banco de dados lance um erro inesperado, por exemplo o MySQL irá devolver este erro:
...
Além de +:conditions+, +Model.find+ aceita uma variedade de outras opções através de um hash, tornando possível personalizar o conjunto de registros resultante.
...
Para recuperar registros do banco de dados em um ordem específica, você pode especificar a opção +:order+ para a chamada +find+.
...
Onde +<attribute>+ é o atributo que você tentou acessar. O método +id+ não lancára +ActiveRecord::MissingAttributeError+, então tome cuidado ao trabalhar com associações porque elas precisam do método +id+ para funcionar corretamente.

Você também pode chamar funçõeas SQL dentro da opção select. Por exemplo, se você quiser pegar somente um único registro para cada valor único em um determinado campo utilizando a função +DISTINCT+, você pode fazer:
...
Ou especificando +:limit+ e +:offset+:
...
Isso irá retornar um único objeto para cada dia, mas somente para o último mês.
...
Qualquer tentativa de alterar ou destruir os registros somente para leitura não terá sucesso, lançando uma exceção +ActiveRecord::ReadOnlyRecord+. Para definir essa opção, especifique-a assim:

<ruby>
Client.first(:readonly => true)
</ruby>

Se você associar este registro com uma variável client, chamar o código a seguir irá lançar uma exceção +ActiveRecord::ReadOnlyRecord+:
...
Travar registros é útil para prevenir race conditions quando estivermos atualizando registros no banco de dados e garantindo atualizações atomicas. O Active Record fornece dois mecanismos de locking:
...
Optimistic locking permite que multiplos usuários acessem o mesmo registro para edição e assume que ocorram poucos conflitos com os dados. Ele faz isso verificando se outro processo realizou mudanças a um registro desde de que ele foi aberto. Uma exceção +ActiveRecord::StaleObjectError+ é lançada caso isso tenha acontecida e a atualização é ignorada.

<strong>Coluna para o Optimistic locking</strong>

Para utilizar o optimistic locking, a tabela precisar ter uma coluna chamada +lock_version+. Cada vez que o registro é atualizado, o Active Record incrementa a coluna +lock_version+ e os recursos de locking garantem que caso um registro seja instanciado duas vezes, o segundo será salvo e uma exceção +ActiveRecord::StaleObjectError+ será lançada caso o primeiro registro também tenha sido atualizado. Exemplo:
...
Você é portanto responsável por lidar com o conflito resgatando a exceção e desfazendo as alterações, mesclando ou então aplicando a lógica de negócio necessária para resolver o conflito.
...
O pessimistic locking utiliza mecanismos de travamento fornecidos pelo próprio banco de dados. Passando +:lock => true+ para +Model.find+ obtém um travamento exclusivo sobre as linhas selecionadas. +Model.find+ utilizando +:lock+ é geralmente colocado dentro de uma transação para prevenir condições de deadlock.
...
Você pode especificar condições nas tabelas associadas usando "Array":#condies-em-arrays e "String":#condies-em-strings-puras comuns. "Hash conditions":#condies-em-hash fornecem uma sintaxe especial para especificar condições para asa tabelas associadas:
...
O código acima encontra a categoria com id 1 e carrega previamente todos os posts associados com a categoria encontrada. Adicionalmente, irá carregar previamente as tags e comentários de cada post. Cada associação entre um comentários e um visitante (guest) será também carregada previamente.

h4. Especificando Condições em Associações Carregadas Previamente

Ainda que o Active Record permita que você especifique condições sobre as associações carregadas previamente assim como ocorre com +:joins+, a forma recomendada é utilizar ":joins":#realizando-joins-entre-tabelas
...
Além disso, +exists+ aceita uma opção +conditions+ de forma semelhante ao +find+:
...
Esta seção utiliza +count+ como um exemplo neste preâmbulo, mas asa opções descritas são aplicáveis à todas as sub-seções.
...
Este código especifica +clients.first_name+ apenas no caso de que uma das tabelas com as quais foi feito join tenha um campo chamado +first_name+ e usa +orders.status+ porque este é o nome da tabela com a qual foi feito o join.
...
Se vocêe quiser ver qual a média de um certo número em uma de suas tabelas você pode chamar o método +average+ sobre a classe relativa à tabela. Essa chamada de método irá se parecer com isso:
...
Se você quiser encontrar o somatório de uma campo para todos os registros da sua tabela você pode chamar o método +sum+ sobre a classe relativa à sua tabela. Essa chamada de método irá se parecer com isso:
