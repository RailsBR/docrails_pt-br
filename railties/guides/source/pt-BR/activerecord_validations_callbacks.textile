...
Esse guia irá ensiná-lo como interceptar eventos que acontecem no ciclo de vida dos seus objetos Active Record. Você vai aprender como validá-los antes de enviá-los para o banco de dados e como realizar operações customizadas em alguns pontos específicos do ciclo de vida do objeto.
...
* Criar Observers que respondem aos eventos do ciclo de vida fora da classe original
...
CAUTION: Existem várias maneiras de alterar o estado de um objeto no banco de dados. Alguns métodos irão disparar validações, mas outros não. Isso significa que é possível salvar um objeto num estado inválido no banco de dados se você não for cuidadoso.
...
Analisaremos os erros de validação mais a fundo na seção "Trabalhando com os Erros de Validação":#trabalhando-com-os-erros-de-validao. Por enquanto, vamos analisar os helpers disponibilizados pelo Rails, por padrão.
...
CAUTION: Não utilize +validates_associated+ nos dois lados da associação, ou eles irão chamar um ao outro e entrar num loop infinito.
...
WARNING: Note que a regular expression acima permite um caractere de fim de linha.
...
WARNING: Tenha em mente que alguns bancos de dados são configurados para realizar pesquisas case-insensitive não importando o opção passada.
...
Existem algumas opções que são comuns a todos os helpers de validação. A seguir explicaremos todas elas, exceto pelas opções +:if+ e +:unless+, que serão discutidas mais adiante na seção "Validações Condicionais":#validao-condicional.
...
Você também pode criar seus próprios helpers de validação e reutilizá-los em diferentes modelos. Nesse exemplo estamos criando um helper de validação customizado para validar o formato de campos que representam um raio (valor de 1 a n):
...
h3. Trabalhando com os Erros de Validação
...
WARNING: O +after_save+ é executado tanto na criação quanto na atualização, mas sempre _depois_ do callbacks mais específicos como +after_create+ e +after_update+, não importando a ordem em que as macros foram executadas.
...
WARNING: Gerar um erro qualquer pode quebrar código que não espera que o +save+ e similares falhem dessa maneira. A exceção +ActiveRecord::Rollback+ foi pensada justamente para dizer ao Active Record que um rollback está sendo executado. Ela é capturada internamente, mas nunca gerada novamente.
...
* April 05, 2009: Callbacks translation revision by "Eleudson Queiroz":credits.html#eleudsonqueiroz
* March 24, 2009: Callbacks translation by "Rafael Rosa":credits.html#rafaelrosafu
