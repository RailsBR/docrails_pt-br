...
Analisaremos os erros de validação mais a fundo na seção "Trabalhando com os Erros de Validação":#trabalhando-com-os-erros-de-validao. Por enquanto, vamos analisar os helpers disponibilizados pelo Rails.
...
Existem algumas opções que são comuns à todas os helpers de validação. A seguir explicaremos todas elas, exceto pelas opções +:if+ e +:unless+, que serão discutidas mais adiante na seção "Validações Condicionais":#validao-condicional.
