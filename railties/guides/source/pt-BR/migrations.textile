...
NOTE: Algumas "ressalvas":#using-models-in-your-migrations se aplicam ao utilizar models nas suas migrations.
...
Se você precisa executar tarefas específicas para seu banco de dados (por exemplo, criar uma "foreign key":#active-recordand-referential-integrity) então a função +execute+ permite que você execute SQL arbitrárias. A migration é apenas uma classe comum em Ruby, então você não está limitado a apenas estas funções. Por exemplo, após adicionar uma coluna, você pode escrever código para setar o valor dessa coluna para dados existentes (se necessário usando seus models).
...
h4. Criando um Model ** Parei aqui - Adolfo **

Um Modelo e os geradores de scaffold criará migrações apropriadas para criação de um novo Modelo. Esta migration contém instruções prontas para criação de uma tabela relevante. Se você mostrar pro Rails as colunas que você precisa então as declarações já estarão criadas. Por exemplo, rodando +ruby script/generate model Product name:string description:text+ gerará uma migration como esta
...
h4. Criando uma migration autônoma

Se você criar uma migration para outros propósitos (Por exemplo para adicionar uma coluna em uma tabela já existente) então você pode usar o gerador de migrações:

<shell>
ruby script/generate migration AddPartNumberToProducts
</shell>

Isto irá criar uma migration vazia mas já apropriada com o nome da migration:
...
Se o nome da migration é na forma AddXXXtoYYY ou RemoveXXXtoYYY e for seguida de uma lista de nomes de colunas e seus tipos então a migration será criada contendo declarações apropriadas para adicionar e remover colunas.
...
Uma vez que você criou uma migration usando um dos geradores, é a hora de trabalhar!
...
a segunda forma, que é chamada de migration "sexy", elimina a redundância dos métodos. Em vez de +string+, +integer+, etc os métodos são criados a partir do tipo da coluna, onde os parâmetros subseqüentes são idênticos.
...
Algmas vezes a sua migration pode fazer algumas coisas irreversíveis, por exemplo quando você destrói alguns dados. Em casos como esse onde você não pode reverter uma migration, você pode lançar IrreversibleMigration para o seu método +down+. Se alguém tentar reverter a sua migration uma mensagem será mostrada falando que ela não será completa.
...
NOTE: Isto não é o mesmo que você rodar todas as migrações - veja a seção "schema.rb":#armazenandoesquemasevoc
