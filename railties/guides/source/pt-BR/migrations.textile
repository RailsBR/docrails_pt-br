...
Se você precisa executar tarefas específicas para seu banco de dados (por exemplo, criar uma "foreign key":#active-recorde-integridade-referencial ) então a função +execute+ permite que você execute SQL arbitrarias. A migração é apenas uma classe regular em Ruby, então você não esta limitado a apenas essas funções. Por exemplo, após adicionar uma coluna, você pode escrever código para setar o valor dessa coluna para dados existentes (se necessário usando seus modelos).
