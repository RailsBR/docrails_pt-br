...
Definindo a opção +:as+ indica que esta será uma associação polimórfica. Associações polimórficas foram discutidas em detalhes <a href="#polymorphic-associations">anteriormente neste guia</a>.
...
A opção +:through+ específica um model para a junção para executar a query. "Associações +has_one :through+ já foram discutidas em detalhes":#a-associaohas-onethrough.
...
Se o objeto pai (o que estiver declarando a associação +has_one+) não é salvo (isto é, +new_record?+ retorna +true+) então os objeto filhos não serão salvos. Eles serão automaticamente salvos quando o pai for.
