...
Para associar um model a outro model em um escopo diferente, você precisa especificar o nome completo da classe na declaração de sua associação:
...
Passando +true+ para a opção +:polymorphic+ indica que esta será uma associação polimórfica. Associações polimórficas foram discutidas em detalhes <a href="#polymorphic-associations">anteriormente neste guia</a>.
...
Definindo a opção +:as+ indica que esta será uma associação polimórfica. Associações polimórficas "já foram discutidas em detalhes neste guia":#associaespolimrficas.
...
<<<<<<< HEAD:railties/guides/source/pt-BR/association_basics.textile
...
>>>>>>> 4e297b239acf7230e13df250e35bdf88dd2c94c9:railties/guides/source/pt-BR/association_basics.textile
...
A opção +:through+ específica um model para a junção (join) para executar a query. "Associações +has_one :through+ já foram discutidas em detalhes":#a-associaohas-onethrough.
...
Definindo a opção +:as+ indica que essa será uma associação polimórfica, "como já discutido previamente neste guia":#associaespolimrficas.
...
A opção +:through+ especifica um model para uma junção. Associações +has_many :through+ criam uma forma de implementar uma associação many-to-many (muitos para muitos), "como discutido anteriormente neste guia":#a-associaohas-manythrough.
...
O método <tt><em>collection</em>.clear</tt> remove cada objeto da coleção através da exclusão das linhas na tabela. Isso não destrói os objetos associados.
...
A opção +:extend+ especifica um módulo para estender o proxy de associação. Extensão de associações "serão discutidas com mais detalhes neste guia":#extensesde-associao.
